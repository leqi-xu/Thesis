...
# Added by Xu: Categerize the triples in training set into 3 groups.
def examples_sampling_score(triples, ent2idx_counts, rel2idx_counts):
    ...
    # Divide equally based on score interval
    ...
    for example, score in zip(ranked_examples, examples_score):
        if score <= threshold1:
    ...
    train1 = easy_examples
    train2 = easy_examples + medium_examples

    return np.array(train1).astype("int64"), np.array(train2).astype("int64"), examples_score, ranked_examples_all
...
def process_dataset(path):
    ...
    for split in splits:
        ...
        if split == "train":
            ...
            train1, train2, examples_score, ranked_examples_all = examples_sampling_score(train_examples, ent2idx_counts, rel2idx_counts)
    ...
    return examples, filters, train1, train2, examples_score, ranked_examples_all
...
if __name__ == "__main__":
    ...
    for dataset_name in os.listdir(data_path):
        if dataset_name == 'FB237_curr':
            ...
            dataset_examples, dataset_filters, train1, train2, examples_score, ranked_examples_all= process_dataset(dataset_path)
            ...
            with open(os.path.join(dataset_path, "train1.pickle"), "wb") as save_file:
                pickle.dump(train1, save_file)
            
            with open(os.path.join(dataset_path, "train2.pickle"), "wb") as save_file:
                pickle.dump(train2, save_file)
