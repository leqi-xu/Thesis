...
parser.add_argument(
    "--dataset", default="FB237_base", choices=["WN18RR_curr", "FB237_curr", "YAGO3-10_curr"],
...
parser.add_argument(
    "--load_model_path", type=str, help="Path to the saved model to load and evaluate"
)
parser.add_argument(
    "--evaluate", action="store_true", help="Whether to only evaluate the model"
)

def train(args):
    save_dir = get_savedir(args.model, args.dataset)

    wandb.init(project='10', entity='xuleqi')
    ...
    train_datasets = ["train1", "train2", "train"]
    dataset_index = 0
    train_examples = dataset.get_examples(train_datasets[dataset_index])
        ...
        if (step + 1) % args.valid == 0:
            ...
            if not best_mrr or valid_mrr > best_mrr:
                ...
                if counter == args.patience:
                    ...
                    # Changed by Xu: Introduce triples with the next difficulty level.
                    dataset_index += 1
                    counter = 0
                    if dataset_index >= len(train_datasets):
                        logging.info("\t All datasets have been used, stopping training")
                        break
                    else:
                        best_mrr = None  # Reset best MRR for next dataset
                        logging.info("\t Switching to next dataset")
                        train_examples = dataset.get_examples(train_datasets[dataset_index])
