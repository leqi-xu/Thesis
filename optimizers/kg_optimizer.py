...
class KGOptimizer(object):
    ...
    # Added by Xu: Sampling with probabilites
    def epoch1(self, examples, probabilities):
        """Runs one epoch of training KG embedding model.
        ...
        Returns:
            loss: torch.Tensor with loss averaged over all training examples
        """
        # Biased sampling: Use the torch.multinomial function to sample triples according to probabilities.
        indices = torch.multinomial(probabilities, examples.shape[0], replacement=False)
        actual_examples = examples[indices, :]
