...
class KGModel(nn.Module, ABC):
   ...
   # 获取头实体、关系和尾实体的 embeddings
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                these_queries = queries[b_begin:b_begin + batch_size].to('cuda')
                ...
                # 从过滤器中获取要忽略的实体，并将正在查询中的正确实体添加到忽略列表中
    ...
    # 计算基于排名的评估指标，包括平均排名（mean rank）、平均倒数排名（mean reciprocal rank）和命中率（hits at k）
