...
parser.add_argument(
    "--dataset", default="FB237_base", choices=["FB237_base", "YAGO3-10_base", "WN18RR_base", "FB237_curr", "YAGO3-10_curr", "FB237_al",],
...
parser.add_argument(
    "--train1", action="store_true", help="Whether use ranked train examples"
)

def train(args):
    save_dir = get_savedir(args.model, args.dataset)

    #Added by Xu: use wandb to record the learning curve
    wandb.init(project='13', entity='xuleqi')
...
def train1(args):
    ...
    # load data
    logging.info("\t " + str(dataset.get_shape()))
    train_examples = dataset.get_ranked_examples()
    valid_examples = dataset.get_examples("valid")
    test_examples = dataset.get_examples("test")
    probabilities = dataset.get_probabilities()
    filters = dataset.get_filters()
    ...
    for step in range(args.max_epochs):
        ...
        # Train step
        model.train()
        # Changed by Xu: use biased sampling
